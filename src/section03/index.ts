...
class GameProcedure {
  ...
  private async play() {
    ...
    //   await this.play()
      ...
      // neverを追加すると、gameのパターンを増やしたときにコンパイルエラーを起こすためバグを検知しやすい
...
class HitAndBlow implements Game {
  ...
  // 正解したら値を初期化する処理を追加
  /*
  抽象クラスで型定義したメソッドを、実装クラスで新たに違う型を指定するとコンパイルエラーになる

  end(v: string) {

  型 'HitAndBlow' のプロパティ 'end' を基本データ型 'Game' の同じプロパティに割り当てることはできません。
  型 '(v: string) => void' を型 '() => Promise<void>' に割り当てることはできません。
